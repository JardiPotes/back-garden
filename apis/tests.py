...
class TestCreateGarden(APITestCase):
    ...
    def test_cannot_create_garden_with_zipcode_too_long(self):
        ...
        self.assertEqual(json_response["zipcode"], ["Ensure this field has no more than 5 characters."])

    def test_cannot_create_garden_with_too_long_title(self):

        random_string_more_than_hundred_char = TestHelper.random_string_more_than_hundred_char(150)
        user = UserFactory.create_user()
        data = GardenFactory.create_garden_dict(user_id=user.id, title=random_string_more_than_hundred_char)
        response = self.client.post('/api/gardens', data, format="json")
        json_response = json.loads(response.content)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(json_response["title"], ["Ensure this field has no more than 100 characters."])
    ...
    def test_cannot_create_garden_if_user_does_not_exist(self):
        ...
        json_response = json.loads(response.content)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(json_response["user_id"], ['Invalid pk "4" - object does not exist.'])


class TestListGardens(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user('hello@world', 'hello_world_123', has_garden=True)
        self.user2 = User.objects.create_user('hey@world.fr', 'hekolololololo', has_garden=True)
        self.garden = Garden.objects.create(user_id=User(id=self.user.id), title="toto", zipcode="75001")
        self.garden2 = Garden.objects.create(user_id=User(id=self.user2.id), title="tata", zipcode="93100")
        self.garden3 = Garden.objects.create(user_id=User(id=self.user2.id), title="titi", zipcode="75001")
